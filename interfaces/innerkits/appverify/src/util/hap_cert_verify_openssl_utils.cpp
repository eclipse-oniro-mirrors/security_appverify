...
X509_CRL* HapCertVerifyOpensslUtils::GetX509CrlFromDerBuffer(
...
bool HapCertVerifyOpensslUtils::GetSerialNumberFromX509(const X509* cert, std::string& certNumber)
{
    if (cert == nullptr) {
        HAPVERIFY_LOG_ERROR("cert is nullptr");
        return false;
    ...
    const ASN1_INTEGER* certSN = X509_get0_serialNumber(cert);
    BIGNUM* bn = ASN1_INTEGER_to_BN(certSN, nullptr);
    if (bn == nullptr) {
        HAPVERIFY_LOG_ERROR("ASN1_INTEGER_to_BN failed");
        return false;
    }
    ...
    HAPVERIFY_LOG_DEBUG("cert number = %{public}s", certNumber.c_str());
    OPENSSL_free(hex);
    BN_free(bn);
    return true;
}
